...
def get_optional_current_user(
    ...
    **الرمز المرسَل يُتحقق منه دائمًا:** الغياب مقبول، أما الرمز التالف أو
    ...
    ⚠️ **لا يستخدمها أي مسار اليوم.** كان `/api/chat` يستخدمها في وضع
    انتقالي، وأُغلق ذلك في P4-03 فصار المسار يعلن `CurrentUser`. تبقى هنا
    للمسارات التي قد تعمل بلا تسجيل دخول مستقبلًا — **ولا تُستعمل في مسار
    يقدّم خدمة فعلية**: استهلاك المودل أو القاعدة بلا هوية ولا سجل تدقيق
    غير مقبول على سيرفر جهة حقيقية.
    """
    if credentials is None or not credentials.credentials.strip():
        return None
