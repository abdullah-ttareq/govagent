 ...
 * يقرأ «مفعّل على جهاز آخر» لا يحتاج زرّ تنبيه: الحل نموذج الاستبدال في
 * الشاشة نفسها.
 ...
 * - `signin` — الجلسة لم تعد صالحة، يسجّل الدخول من جديد.
 * - `none` — لا إجراء يفيده (اشتراك منتهٍ، جهاز آخر، تجهيز ناقص).
 */

/**
 * رسالة ٤٠١ حين لا يرسل السيرفر تفصيلًا.
 *
 * ⚠️ **لا تقول «انتهت جلستك».** ٤٠١ على مسار محمي قد تعني رمزًا لم يعد
 * السيرفر يقبله لسبب لا علاقة له بالوقت — وقد تقع **بعد ثوانٍ من تسجيل
 * ناجح**، فتُقرأ حينها على أنها «بريدك أو كلمة مرورك خطأ» وهي ليست كذلك.
 * فالنصّ ينفي ذلك صراحةً بدل أن يترك المستخدم يخمّن.
 */
export const SESSION_UNUSABLE_MESSAGE =
  "تعذّر التحقق من جلستك على السيرفر. بريدك وكلمة مرورك سليمان — سجّل " +
  "الدخول مرة أخرى للمتابعة.";
...
export function describeFailure(caught, fallback = "حدث خطأ غير متوقع.") {
  ...
  if (caught instanceof ApiError) {
    switch (caught.status) {
      case 401:
        // **رسالة السيرفر أدقّ من أي عبارة عامة هنا**: هو وحده يعرف
        // أانتهت الصلاحية، أم صار الرمز غير مقبول، أم جاء من مشروع آخر.
        // ولا يُقال «انتهت جلستك» إلا إن قالها هو.
        return {
          message: caught.detail || SESSION_UNUSABLE_MESSAGE,
          action: "signin",
          sessionLost: true,
        };
      case 403:
        // اشتراك منتهٍ أو موقوف، أو حساب ناقص التجهيز. رسالة
        // الـBackend تذكر السبب وتاريخ الانتهاء، فلا تُستبدل بعبارة عامة.
        return { message: caught.message, action: "none", sessionLost: false };
      case 409:
        // مفعّل على جهاز آخر، أو لا جهاز مفعّلًا. الحل في الشاشة نفسها.
        return { message: caught.message, action: "none", sessionLost: false };
      case 400:
      case 404:
      case 422:
      ...
      case 503:
        ...
        // المتغيّر الناقص، وهي ما يحتاجه من ينشر النظام حرفيًا.
        return { message: caught.message, action: "retry", sessionLost: false };
      default:
        if (caught.status >= 500) {
          return {
            message:
              "حدث خطأ في خدمة GovMind. حاول مرة أخرى، وإن تكرر فتواصل مع الدعم.",
            action: "retry",
            sessionLost: false,
          };
        }
